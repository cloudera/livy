...
class LdapAuthenticationHandlerImpl() extends AuthenticationHandler {
  ...
  @VisibleForTesting def setEnableStartTls(enableStartTls: Boolean): Unit = this.enableStartTls = enableStartTls

  @VisibleForTesting def setDisableHostNameVerification(disableHostNameVerification: Boolean): Unit = this.disableHostNameVerification = disableHostNameVerification
  ...
  def init(config: Properties): Unit = {
    ...
    this.enableStartTls = config.getProperty(LdapAuthenticationHandlerImpl.ENABLE_START_TLS, "false").toBoolean
  ...
  def managementOperation(token: AuthenticationToken, request: HttpServletRequest, response: HttpServletResponse) : Boolean = true

  @throws[IOException]
  @throws[AuthenticationException]
  def authenticate(request: HttpServletRequest, response: HttpServletResponse): AuthenticationToken = {
    var token: AuthenticationToken = null
    var authorization = request.getHeader("Authorization")
    if (authorization != null && authorization.regionMatches(true, 0, LdapAuthenticationHandlerImpl.AUTHORIZATION_SCHEME, 0, LdapAuthenticationHandlerImpl.AUTHORIZATION_SCHEME.length)) {
      authorization = authorization.substring("Basic".length).trim
      val base64 = new Base64(0)
      val credentials = new String(base64.decode(authorization), StandardCharsets.UTF_8).split(":", 2)
      ...
      else LdapAuthenticationHandlerImpl.logger.warn("\'Authorization\' does not start with \'Basic\' :  {}", authorization)
    }
    token
  }

  @throws[AuthenticationException]
  private def authenticateUser(userName: String, password: String) = if (userName != null && !userName.isEmpty) {
    var principle: String = userName
    if (!LdapAuthenticationHandlerImpl.hasDomain(userName) && this.ldapDomain != null) principle = userName + "@" + this.ldapDomain
    ...
    else throw new AuthenticationException("Error validating LDAP user: a null or blank password has been provided")
  }
  else throw new AuthenticationException("Error validating LDAP user: a null or blank username has been provided")
  ...
  private def authenticateWithTlsExtension(userDN: String, password: String) = {
    ...
    try {
      ...
      if (this.disableHostNameVerification.booleanValue) ex.setHostnameVerifier(new HostnameVerifier() {
        override def verify(hostname: String, session: SSLSession) = true
      })
      ex.negotiate
      ctx.addToEnvironment("java.naming.security.authentication", LdapAuthenticationHandlerImpl.SECURITY_AUTHENTICATION)
  ...
  private def authenticateWithoutTlsExtension(userDN: String, password: String) = {
    ...
    env.put("java.naming.factory.initial", "com.sun.jndi.ldap.LdapCtxFactory")
    env.put("java.naming.provider.url", this.providerUrl)
    env.put("java.naming.security.authentication", LdapAuthenticationHandlerImpl.SECURITY_AUTHENTICATION)
